...
document.addEventListener('DOMContentLoaded', () => {
    ...
    var pwaName = "Ticketsmarche"; //Local Storage Names for PWA
    var pwaRemind = 1; //Days to re-remind to add to home
    var pwaNoCache = false; //Requires server and HTTPS/SSL. Will clear cache with each visit
    
    //Setting Service Worker Locations scope = folder | location = service worker js location
    var pwaScope = "https://app.ticketsmarche.com/17f95c00-4ab0-492d-94a6-3a647e5ea2fe";
    var pwaLocation = "https://app.ticketsmarche.com/17f95c00-4ab0-492d-94a6-3a647e5ea2fe/_service-worker.js";
    ...
    function init_template(){
        ...
        if(inputField.length){      
            var mailValidator = /^[A-Z0-9._%+-]+@([A-Z0-9-]+\.)+[A-Z]{2,20}$/i;
            var phoneValidator = /^[(]{0,1}[0-9]{3}[)]{0,1}[-\s\.]{0,1}[0-9]{3}[-\s\.]{0,1}[0-9]{4,5}$/;
		...
		//Clear search filter
        var clearFilter = document.getElementsByClassName('clear_filter');
        if(clearFilter.length){
			Array.prototype.forEach.call(clearFilter, function(e) {
			//clearFilter.forEach(function(e){
				if(e.hasAttribute('data-input')){
					var clearInput = document.getElementById(e.getAttribute('data-input'));
					var submitForm = document.getElementById(e.getAttribute('data-form'));
		...
		//Card Rotate 
        var cardRotateManual = document.querySelectorAll('[data-card="card_1"]');
		//cardRotateManual = false;
        if(cardRotateManual.length){
			cardRotateManual.forEach(function(e){
				var flipButtonAttr = e.getAttribute("data-button");
				var cardAttr = e.getAttribute("data-card");
				var flipButtonFront = document.getElementById(flipButtonAttr +"_front");
				var flipButtonBack = document.getElementById(flipButtonAttr +"_back");

				if(flipButtonFront){
					flipButtonFront.addEventListener('click', e => {
						var cardContainer = document.getElementById("card-flip");
				...
				if(e){
					flipButtonBack.addEventListener('click', e => {
						var cardContainer = document.getElementById("card-flip");
        ...
        //Image Sliders
		var splideAttr = document.querySelectorAll('[data-splide-mount]');
        if(splideAttr.length){            
			splideAttr.forEach(function(e){
				var sliderID = e.getAttribute('data-splide-mount');
				
				e.addEventListener('click', e => {
					var sliderElement = document.getElementById(sliderID);
					
					var splideType = 'loop';
					if(sliderElement.hasAttribute('data-splidetype')){
						splideType = sliderElement.getAttribute('data-splidetype');
					}					
					
					var single = new Splide( '#'+sliderElement.id, {
						type:splideType,
						autoplay:false,
						interval:4000,
						perPage: 1,
        ...
        if(splide.length){            
            ...
            if(singleSlider.length){
                singleSlider.forEach(function(e){
					var splideType = 'loop';
					if(e.hasAttribute('data-splidetype')){
						splideType = e.getAttribute('data-splidetype');
					}					
            ...
            var singleSlideSlider = document.querySelectorAll('.single-slider-slide');
            if(singleSlideSlider.length){
                singleSlideSlider.forEach(function(e){
                    var single = new Splide( '#'+e.id, {
                        type:'slide',
                        autoplay:false,
                        interval:4000,
        ...
        if(registerForm.length){
			//Ensure we hide any previous errors
			document.querySelectorAll('.register-validator').forEach(el=>{el.classList.add('disabled');})
            ...
            form.onsubmit = function (e) {
                ...
                var mobileField = document.getElementById('registerMobile');
                var passwordField = document.getElementById('registerPassword');
                var validateMail = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,20})?$/;
                if(emailField.value === ''){
                    form.setAttribute('data-form','invalid');
                    emailField.classList.add('border-red-dark');
                ...
                if(emailField.value === ''){
                    ...
                    if(!mailValidator.test(emailField.value)){
                        form.setAttribute('data-form','invalid');
                        emailField.classList.add('border-red-dark');
                        ...
                        emailField.classList.remove('border-red-dark');
                    }     
                }
        ...
        if(loginForm.length){
			//Ensure we hide any previous errors
			document.querySelectorAll('.login-validator').forEach(el=>{el.classList.add('disabled');})
        ...
        var forgotForm = document.querySelectorAll('.forgot-form');
        if(forgotForm.length){
			//Ensure we hide any previous errors
			document.querySelectorAll('.forgot-validator').forEach(el=>{el.classList.add('disabled');})
        ...
        if(resetForm.length){
			//Ensure we hide any previous errors
			document.querySelectorAll('.reset-validator').forEach(el=>{el.classList.add('disabled');})
            ...
            form.onsubmit = function (e) {
                ...
                var passwordConfirmField = document.getElementById('new_confirm');
                
                if(passwordField.value === ''){
                    form.setAttribute('data-form','invalid');
                    passwordField.classList.add('border-red-dark');
                    document.getElementById('validator-password1').classList.remove('disabled');
					formOk = false;
                } 

                if(passwordConfirmField.value === ''){
                    form.setAttribute('data-form','invalid');
                    passwordConfirmField.classList.add('border-red-dark');
                    document.getElementById('validator-password2').classList.remove('disabled');
					formOk = false;
                } 
        ...
        //Transfer Form
        var transfernewForm = document.querySelectorAll('.transfernew-form');
        if(transfernewForm.length){
			//Ensure we hide any previous errors
			document.querySelectorAll('.transfernew-validator').forEach(el=>{el.classList.add('disabled');})
			
            var form = document.getElementById('transferNewForm');
            form.onsubmit = function (e) {
				var formOk = true;

				var share_ticketChk = document.querySelector('#share_ticket');
				if(share_ticketChk.checked){
					document.getElementById("transfer_ticket_share").value = 1;
				}
                ...
                var lastName = document.getElementById('lastName');
                var emailField = document.getElementById('emailAddress');
                var mobileField = document.getElementById('transfernewMobile');
                var validateMail = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,20})?$/;
                if(emailField.value === ''){
                    form.setAttribute('data-form','invalid');
                    emailField.classList.add('border-red-dark');
        ...
        //Transfer Multiple Form
        var transfernewForm = document.querySelectorAll('.transfernewmul-form');
        if(transfernewForm.length){
			//Ensure we hide any previous errors
			document.querySelectorAll('.transfernewmul-validator').forEach(el=>{el.classList.add('disabled');})
			
            var form = document.getElementById('transferMulForm');
            form.onsubmit = function (e) {
				var formOk = true;

                //Validate Fields
                var transfer_account_uuid = document.getElementById('transfer_account_uuid');
				if(transfer_account_uuid == ""){				
					var firstName = document.getElementById('firstName');
					var lastName = document.getElementById('lastName');
					var emailField = document.getElementById('emailAddress');
					var mobileField = document.getElementById('transfernewMobile');
        ...
        //Profile Form
        var profileForm = document.querySelectorAll('.profile-form');
        if(profileForm.length){
			//Ensure we hide any previous errors
			document.querySelectorAll('.profile-form-validator').forEach(el=>{el.classList.add('disabled');})
            ...
            form.onsubmit = function (e) {
                ...
                if(emailField.value === ''){
                    ...
                    document.getElementById('validator-mail1').classList.add('disabled');
                    if(!validateMail.test(emailField.value)){
                        form.setAttribute('data-form','invalid');
                        emailField.classList.add('border-red-dark');
                        document.getElementById('validator-mail2').classList.remove('disabled');
						formOk = false;
		...
		//Address Form
        var addressForm = document.querySelectorAll('.address-form');
        if(addressForm.length){
			//Ensure we hide any previous errors
			document.querySelectorAll('.address-form-validator').forEach(el=>{el.classList.add('disabled');})
        ...
        var transfer_account = document.querySelectorAll('.transfer_account');
        transfer_account.forEach(el => el.addEventListener('click', e => {
            var accountUUID = el.getAttribute('data-account_uuid');
            var formID = el.getAttribute('data-form');
			document.getElementById("transfer_account_uuid").value = accountUUID;
			
			var share_ticketChk = document.querySelector('#share_ticket');
			if(share_ticketChk != null && share_ticketChk.checked){
				document.getElementById("transfer_ticket_share_f").value = 1;
			}
        ...
        var cancel_transfer = document.querySelectorAll('.cancel_transfer');
        cancel_transfer.forEach(el => el.addEventListener('click', e => {			
			var form = document.getElementById('cancelFFForm');
			form.submit();
        }));
        
		//Add New FandF
        var newFFForm = document.querySelectorAll('.newffform-form');
        if(newFFForm.length){
			//Ensure we hide any previous errors
			document.querySelectorAll('.newffform-validator').forEach(el=>{el.classList.add('disabled');})
			
            var form = document.getElementById('newFFForm');
            form.onsubmit = function (e) {
				var formOk = true;
		...
		var remove_fandf = document.querySelectorAll('.remove_fandf');
		remove_fandf.forEach(el => el.addEventListener('click', e => {
			var accountUUID = el.getAttribute('data-account_uuid');
			var formID = el.getAttribute('data-form');
			document.getElementById("remove_account_uuid").value = accountUUID;
        ...
        if(contactForm.length){
            ...
            form.onsubmit = function (e) {
                ...
                if(mailField.value === ''){
                    ...
                    document.getElementById('validator-mail1').classList.add('disabled');
                    if(!mailValidator.test(mailField.value)){
                        form.setAttribute('data-form','invalid');
                        mailField.classList.add('border-red-dark');
                        document.getElementById('validator-mail2').classList.remove('disabled');
                    } else{
